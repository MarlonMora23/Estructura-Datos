...
def quick_sort(data, first, last, condition, reverse) -> list:
    ...
    while left <= right:
        while left <= right and data[left][condition] <= data[pivot][condition]:
            left += 1
        while right >= left and data[right][condition] >= data[pivot][condition]:
            right -= 1
        if left < right:
...
def count_sort_numeric(data, condition, max_value) -> list:
    count_list = [0] * (max_value + 1)
    sorted_list = [None] * len(data)

    for i in data:
        rounded_condition = int(round(i[condition]))  # Redondear a entero
        count_list[rounded_condition] += 1
    ...
    for index in data:
        rounded_condition = int(round(index[condition]))  # Redondear a entero
        sorted_list[count_list[rounded_condition]] = index
        count_list[rounded_condition] += 1

    return sorted_list

def count_sort_alphabetic(data, condition, _) -> list:
    max_value = 256  
    count_list = [0] * (max_value + 1)
    sorted_list = [None] * len(data) 

    for i in data:
        count_list[ord(i[condition][0])] += 1  
    ...
    for index in data:
        sorted_list[count_list[ord(index[condition][0])]] = index
        count_list[ord(index[condition][0])] += 1
...
def handle_count_sort(data, condition, reverse=False) -> list:
    ...
    # Verificar si max_value es numérico o alfabético
    if type(max_value) is str:
        result = count_sort_alphabetic(data, condition, max_value)
    else:
        result = count_sort_numeric(data, condition, max_value)

    return result if not reverse else result[::-1]

def bucket_sort_numeric(data, condition) -> list:
    # Determinar el número de cubetas
    num_buckets = 10
    min_value = min(element[condition] for element in data)
    max_value = max(element[condition] for element in data)
    
    # Distribuir elementos en las cubetas
    buckets = [[] for _ in range(num_buckets)]
    for element in data:
        normalized_value = (element[condition] - min_value) / (max_value - min_value)
        bucket_index = int(normalized_value * (num_buckets - 1))
        buckets[bucket_index].append(element)

    # Ordenar cada cubeta usando otro algoritmo (usaremos sorted en este caso)
...
def bucket_sort_alphabetic(data, condition) -> list:
    # Determinar el rango de valores alfabéticos
    unique_values = sorted(set(element[condition] for element in data))

    # Distribuir elementos en las cubetas
    num_buckets = len(unique_values)
    buckets = [[] for _ in range(num_buckets)]
    for element in data:
        bucket_index = unique_values.index(element[condition])
        buckets[bucket_index].append(element)

    # Ordenar cada cubeta usando sorted
    for i in range(num_buckets):
        buckets[i] = sorted(buckets[i], key=lambda x: x[condition])
...
def bucket_sort(data, condition, reverse=False) -> list:
    if type(data[0][condition]) is str:
        result = bucket_sort_alphabetic(data, condition)
    else:
        result = bucket_sort_numeric(data, condition)
...
def select_sort_algorithm() -> callable:
    print('\n' + '-'*50)
    print(f'{"Algoritmos de ordenamiento":^50}')
    print('-'*50 + '\n')
    #Obtener algoritmos de ordenamiento
